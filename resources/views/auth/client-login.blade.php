...
<div class="flex min-h-screen font-sans flex-col lg:flex-row">
    ...
    <div class="w-full lg:w-1/2 min-h-screen flex flex-col px-4 sm:px-8 lg:px-20 py-6 sm:py-10 bg-white">
        ...
        <div class="flex-1 flex flex-col justify-center max-w-md mx-auto w-full">
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-4 sm:space-y-6" onsubmit="handleLoginSubmit(event)">
                @csrf
                @if(request('redirect'))
                    <input type="hidden" name="redirect" value="{{ request('redirect') }}" id="redirect-url">
                @endif
                ...
                <div>
                    ...
                    <div class="flex justify-end mt-1.5 sm:mt-2">
                        <a href="{{ route('password.request', ['role' => 'client']) }}" class="text-xs sm:text-sm font-bold text-[#f44e08] hover:underline">Forgot password?</a>
                    </div>
                </div>
