...
<div class="flex min-h-screen font-sans">
    ...
    <div class="w-full lg:w-1/2 min-h-screen flex flex-col px-8 lg:px-20 py-10 bg-white overflow-y-auto">
        ...
        <div class="flex-1 flex flex-col justify-center max-w-md mx-auto w-full pb-10">
            ...
            <form method="POST" action="{{ route('caterer.register') }}" class="space-y-5" onsubmit="return handlePhoneSubmit()">
                @csrf
                <div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-900 text-base font-medium">+63</span>
                        <input type="tel" id="phone" name="phone" value="{{ old('phone') ? ltrim(str_replace('+63', '', old('phone'))) : '' }}" required placeholder="9123456789" maxlength="10" pattern="9[0-9]{9}"
                            class="w-full pl-16 pr-4 py-3.5 rounded-xl bg-gray-50 border {{ $errors->has('phone') ? 'border-red-400' : 'border-gray-200' }} focus:outline-none focus:ring-2 focus:ring-[#111827] transition-all"
