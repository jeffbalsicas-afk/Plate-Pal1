...
>
    ...
    <div class="bg-white rounded-2xl border border-[#EDE4D8] overflow-hidden flex flex-col h-96">
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-4">
            @forelse($messages as $message)
                <div class="flex {{ $message->sender === auth()->user()->role ? 'justify-end' : 'justify-start' }}">
                    <div class="max-w-xs {{ $message->sender === auth()->user()->role ? 'bg-[#E8642A] text-white' : 'bg-[#FDF6EE] text-[#1C1A17]' }} rounded-lg px-4 py-2">
                        <p class="text-sm">{{ $message->body }}</p>
                        <p class="text-xs {{ $message->sender === auth()->user()->role ? 'text-orange-100' : 'text-[#8A7F72]' }} mt-1">
        ...
        <div class="border-t border-[#EDE4D8] p-6">
            <form action="{{ route('messages.store', $recipient) }}" method="POST" class="flex gap-3">
                @csrf
                ...
                <button type="submit" class="px-6 py-2 rounded-lg bg-[#E8642A] text-white text-sm font-bold hover:bg-[#F07C42] transition-colors">
                    Send
                </button>
            </form>
        </div>
