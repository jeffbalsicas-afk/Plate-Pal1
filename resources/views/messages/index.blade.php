...
>
    ...
    <div class="grid lg:grid-cols-3 gap-6">
                    ...
                    @forelse($conversations as $conversation)
                        ...
                        <a href="{{ route('messages.show', $recipient) }}" class="flex items-center gap-3 p-4 hover:bg-[#FDF6EE] transition-colors">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-[#8A7F72] truncate">{{ Str::limit($lastMessage->previewText(), 40) }}</p>
                            </div>
                        </a>
                    @empty
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl border border-[#EDE4D8] overflow-hidden">
                <div class="p-6 border-b border-[#EDE4D8]">
                    <h2 class="text-lg font-black text-[#1C1A17]">Start a Conversation</h2>
                    <p class="text-sm text-[#8A7F72] mt-1">
                        @if($user->role === 'client')
                            Message approved caterers about your event plans.
                        @else
                            Message clients who have sent booking requests.
                        @endif
                    </p>
                </div>
                ...
                @if($contactSuggestions->isEmpty())
                    <div class="p-10 text-center">
                        <svg class="size-14 text-[#D3CCBE] mx-auto mb-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z"/></svg>
                        <p class="text-sm font-medium text-[#8A7F72]">
                            @if($user->role === 'client')
                                No new caterers to message from here.
                            @else
                                Clients will appear here after they create bookings.
                            @endif
                        </p>
                        <a href="{{ $user->role === 'client' ? route('client.browse') : route('caterer.bookings') }}" class="mt-4 inline-flex justify-center px-5 py-2.5 rounded-xl bg-[#E8642A] text-white text-sm font-bold hover:bg-[#F07C42] transition-colors">
                            {{ $user->role === 'client' ? 'Browse Caterers' : 'View Bookings' }}
                        </a>
                    </div>
                @else
                    <div class="grid sm:grid-cols-2 gap-3 p-6">
                        @foreach($contactSuggestions as $contact)
                            <a href="{{ route('messages.show', $contact) }}" class="rounded-xl border border-[#EDE4D8] p-4 hover:bg-[#FDF6EE] transition-colors">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-[#E8642A] text-white text-xs font-bold flex items-center justify-center flex-shrink-0">
                                        {{ strtoupper(substr($contact->business_name ?? $contact->name, 0, 1)) }}
                                    </div>
                                    <div class="min-w-0">
                                        <h3 class="font-bold text-[#1C1A17] truncate">{{ $contact->business_name ?? $contact->name }}</h3>
                                        <p class="text-xs text-[#8A7F72] truncate">
                                            @if($contact->role === 'caterer')
                                                {{ $contact->barangay ?? 'Tagum City' }} · {{ $contact->cuisine ?? 'Catering Services' }}
                                            @else
                                                {{ $contact->email }}
                                            @endif
                                        </p>
                                ...
                                <div class="mt-3 text-xs font-bold text-[#E8642A]">Message</div>
                            </a>
                        @endforeach
                    </div>
                @endif
