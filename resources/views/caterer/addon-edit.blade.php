...
>
    <x-slot:sidebar>
        ...
        <a href="{{ route('caterer.reviews') }}" class="flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-[#1C1A17] hover:bg-[#FDF6EE] transition-colors text-sm font-medium">
            <svg class="size-4 stroke-[#8A7F72]" fill="none" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07( round" stroke-linejoin="round" d="M8
            Reviews
        </a>
            ...
            <form method="POST" action="{{ route('menu.addons.update', $addon) }}" class="p-6 sm:p-8 space-y-6">
                ...
                <div>
                    ...
                    <select
                        ...
                        class="w-full px-4 py-3 rounded-xl bg-[#FDF6EE] border border-[#EDE4D8] text-sm text-[#1C1A17] focus:outline-none focus:border-[#E8642A]"
                    >
                        <option value="draft" @selected(old('status', $addon->status) === 'draft')>Draft</option>
                        <option value="live" @selected(old('status', $addon->status) === 'live')>Live</option>
                    </select>
                    @error('status')
                        <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                    @enderror
