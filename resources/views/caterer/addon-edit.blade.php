...
>
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white rounded-2xl border border-[#EDE4D8] shadow-lg overflow-hidden">
            ...
            <form method="POST" action="{{ route('menu.addons.update', $addon) }}" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <img src="{{ $addon->image_path }}" alt="{{ $addon->name }}" class="w-32 h-32 object-cover rounded-xl border border-[#EDE4D8]">
                </div>
                @endif

                <div x-data="{ fileName: '' }">
                    <label class="block text-sm font-bold text-[#1C1A17] mb-2">{{ $addon->image_path ? 'Change Image' : 'Add Image' }} (Optional)</label>
