    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white rounded-2xl border border-[#EDE4D8] shadow-lg overflow-hidden">
            <div class="p-6 sm:p-8 border-b border-[#EDE4D8]">
                <h1 class="text-3xl sm:text-4xl font-black text-[#1C1A17] mb-2">Edit Menu Item</h1>
                <p class="text-base sm:text-lg text-[#8A6D3F]">Update your dish details</p>
            </div>

            <form method="POST" action="{{ route('menu.items.update', $item) }}" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
                @csrf
                @method('PUT')

                @if($item->image_path)
                <div>
                    <label class="block text-sm font-bold text-[#1C1A17] mb-2">Current Image</label>
                    <img src="{{ $item->image_path }}" alt="{{ $item->name }}" class="w-32 h-32 object-cover rounded-xl border border-[#EDE4D8]">
                </div>
                @endif

                <div x-data="{ fileName: '' }">
                    <label class="block text-sm font-bold text-[#1C1A17] mb-2">{{ $item->image_path ? 'Change Image' : 'Add Image' }} (Optional)</label>
                ...
                <div>
                    <label for="name" class="block text-sm font-bold text-[#1C1A17] mb-2">Item Name *</label>
                    <input
                        id="name"
                        name="name"
                        type="text"
                ...
                <div class="grid sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        >
                            ...
                            <option value="head" @selected(old('unit', $item->unit) === 'head')>Per Head</option>
                            <option value="tray" @selected(old('unit', $item->unit) === 'tray')>Per Tray</option>
                            <option value="whole" @selected(old('unit', $item->unit) === 'whole')>Whole</option>
                            <option value="bottle" @selected(old('unit', $item->unit) === 'bottle')>Per Bottle</option>
                            <option value="box" @selected(old('unit', $item->unit) === 'box')>Per Box</option>
                        </select>
                        @error('unit')
                            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    <label for="category" class="block text-sm font-bold text-[#1C1A17] mb-2">Category *</label>
                    <select
                        id="category"
                        name="category"
                        required
                    ...
                    >
                        ...
                        <option value="main" @selected(old('category', $item->category) === 'main')>Main Dish</option>
                        <option value="side" @selected(old('category', $item->category) === 'side')>Side Dish</option>
                        <option value="dessert" @selected(old('category', $item->category) === 'dessert')>Dessert</option>
                        <option value="beverage" @selected(old('category', $item->category) === 'beverage')>Beverage</option>
                    </select>
                    @error('category')
                        <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                    @enderror
