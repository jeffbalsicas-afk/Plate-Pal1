    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white rounded-2xl border border-[#EDE4D8] shadow-lg overflow-hidden">
            <div class="p-6 sm:p-8 border-b border-[#EDE4D8]">
                <h1 class="text-3xl sm:text-4xl font-black text-[#1C1A17] mb-2">Edit Package</h1>
                <p class="text-base sm:text-lg text-[#8A6D3F]">Update your package details</p>
            </div>

            <form method="POST" action="{{ route('menu.packages.update', $package) }}" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <img src="{{ $package->image_path }}" alt="{{ $package->name }}" class="w-32 h-32 object-cover rounded-xl border border-[#EDE4D8]">
                </div>
                @endif

                <div x-data="{ fileName: '' }">
                    <label class="block text-sm font-bold text-[#1C1A17] mb-2">{{ $package->image_path ? 'Change Image' : 'Add Image' }} (Optional)</label>
                ...
                <div>
                    <label for="name" class="block text-sm font-bold text-[#1C1A17] mb-2">Package Name *</label>
                    <input
                        id="name"
                        name="name"
                        type="text"
                ...
                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        <label for="price" class="block text-sm font-bold text-[#1C1A17] mb-2">Package Price *</label>
                        <input
                            id="price"
                            name="price"
                            type="number"
                    ...
                    <div>
                        <label for="min_guests" class="block text-sm font-bold text-[#1C1A17] mb-2">Minimum Guests *</label>
                        <input
                            id="min_guests"
                            name="min_guests"
                            type="number"
